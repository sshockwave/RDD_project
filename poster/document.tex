...
In this section we do not consider covariates. We generate data by different $X-Y$ relation and different distribution of samples. (See table \ref{table:kernels_test})
...
\begin{figure}
   \includegraphics{case30_frame0000000.png}
   \includegraphics{case30_frame0000031.png}
   \includegraphics{case30_frame0000049.png}
   \caption{Illustrated best kernels under different bandwidths}
   \label{fig:kernels}
\end{figure}

A typical result(case30) is shown in Fig.\ref{fig:kernels}. The $x$-axis is the distance from the cutoff, and the $y$-axis is the weight of the kernel. The kernel first chooses to spread out the weight evenly so that more data are used to lower the variance of estimation. As the bandwidth $b$ becomes larger, the weights concentrate to the cutoff to lower the bias caused by the nonlinearity in $X-Y$ relation.
