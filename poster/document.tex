...
Such effect appears as a sudden increase or decrease of $y$ in the neighborhood of $X=t$. Therefore, $\tau$ is traditionally estimated using the see effect of $W$. Formally put,
...
However, we are lack of data near the threshold in most cases, difficult for us to calculate these values on the threshold directly. Thus we need some sort of regression to infer the relation between $X$ and $Y$ on both sides so as to predict $E[Y|X=t_+]$ and $E[Y|X=t_-]$.
